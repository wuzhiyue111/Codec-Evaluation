...
class PromptCondAudioDiffusion(nn.Module):
    def __init__(
        ...
        uncondition=True,
        encoder_ckpt_path = "",
        content_vec_ckpt_path ="/algo-intern/user/shengming/CKPT/levo/models--lengyue233--content-vec-best/snapshots/c0b9ba13db21beaa4053faae94c102ebe326fd68",
        out_paint=False,
    ):
        super().__init__()
        ...
        self.bestrq = load_model(
            model_dir='levo_modules/Flow1dVAE/our_MERT_BESTRQ/mert_fairseq',
            checkpoint_dir=encoder_ckpt_path,
        )
        ...
        # self.hubert = HubertModelWithFinalProj.from_pretrained("/algo-intern/user/shengming/CKPT/models--lengyue233--content-vec-best/snapshots/c0b9ba13db21beaa4053faae94c102ebe326fd68")
        self.hubert = HubertModelWithFinalProj.from_pretrained(content_vec_ckpt_path)

        print(f"self.hubert initialized from pretrain.")
    ...
    def inference_codes(self, codes, spk_embeds, true_latents, latent_length, additional_feats,incontext_length=127, 
        ...
        # device = self.device
        device = next(iter(self.rsp48toclap.state_dict().values())).device
