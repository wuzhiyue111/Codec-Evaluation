...
"""QwenSVDVQ (SVD Vector-Quantized Audio Model using Qwen2Audio)."""
...
from svd_vq_model import Qwen2Audio_SVDVQ  # type: ignore
from svd_vq_lit_model import Qwen2Audio_SVDVQ_LitModel  # type: ignore


__all__ = ["QwenSVDVQ"]


class QwenSVDVQ(Codec):
    def __init__(
        self,
        sample_rate,
        mode="reconstruct",
        ...
        if model_ckpt_path:
            if os.path.exists(model_ckpt_path):
                # Load LitModel checkpoint and extract SVDVQ model from it
                try:
                    # First try to load complete LitModel
                    lit_model = Qwen2Audio_SVDVQ_LitModel.load_from_checkpoint(
                        model_ckpt_path,
                        map_location='cpu'
                    )
                    self.model = lit_model.model  # Extract inner SVDVQ model
                    ...
                    self.model = Qwen2Audio_SVDVQ(model_config)
            ...
            raise ValueError("model_ckpt_path is required for QwenSVDVQ")
        
        # Load Qwen2Audio model and copy pretrained encoder weights
    ...
    def _sig_to_unquantized_emb(self, sig, length):
        """
            sig: [B, T]
            return: [B, D, N]    [batch_size, svd_output_dim, sequence_length]
        """
        # Process audio to mel features and get attention mask
        input_features, attention_mask = self.process_sig(sig, length)
        
        # Use model to get unquantized embeddings using the mel_to_embed method
        with torch.no_grad():
            # Use mel_to_embed with quantized=False to get SVD features
            ...
            # Get SVD features from result
            svd_features = result["embeddings"]  # [B, T_enc, svd_output_dim]
        
        # Transpose to expected output format [B, D, N]
        unquantized_emb = svd_features.transpose(1, 2)
    ...
    def _sig_to_quantized_emb(self, sig, length, use_decoder=False):
        ...
        # Use model to get quantized embeddings using the mel_to_embed method
        with torch.no_grad():
    ...
    def _sig_to_toks(self, sig, length):
        ...
        # SVDVQ only has one codebook, so K=1
    ...
    def _toks_to_sig(self, toks, length, padding_mask=None):
        """
            ...
            This method is not implemented for QwenSVDVQ as it doesn't support
            direct decoding to waveform.
        """
        raise NotImplementedError("QwenSVDVQ doesn't support decoding tokens to audio signals")
...
if __name__ == "__main__":
    ...
    if os.path.exists(audio_path):
        ...
        ckpt_path = "/home/lr/project/Echodec/sfm/train/whisperVQ_train/experiments/Qwen2_SVDVQ_256_code_256_learnFalse/checkpoints/last.ckpt"  # Replace with actual path
