...
class MTGTop50dataset(Dataset):
    ...
    def read_class2id(self):
        ...
        for split in ['train', 'validation']:
            dataset = load_from_disk(os.path.join(os.path.dirname(self.dataset_path), f"MTGTop50_{split}_dataset"))
...
class MTGTop50Module(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            self.train_dataset = MTGTop50dataset(dataset_path=self.train_audio_dir, **self.dataset_args)
            self.val_dataset = MTGTop50dataset(dataset_path=self.val_audio_dir, **self.dataset_args)
        if stage == "val":
            self.val_dataset = MTGTop50dataset(dataset_path=self.val_audio_dir, **self.dataset_args)
        if stage == "test":
            self.test_dataset = MTGTop50dataset(dataset_path=self.test_audio_dir, **self.dataset_args)
