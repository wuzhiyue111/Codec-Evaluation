...
class VocalSetdataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            self.train_dataset = VocalSetTechdataset(dataset_path=self.train_audio_dir, **self.dataset_args)  
            self.val_dataset = VocalSetTechdataset(dataset_path=self.val_audio_dir, **self.dataset_args) 
        if stage == "val":
            self.val_dataset = VocalSetTechdataset(dataset_path=self.val_audio_dir, **self.dataset_args) 
        if stage == "test":
            self.test_dataset = VocalSetTechdataset(dataset_path=self.test_audio_dir, **self.dataset_args)
