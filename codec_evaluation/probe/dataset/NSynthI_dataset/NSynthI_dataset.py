...
class NSynthIdataset(Dataset):
    ...
    def get_item(self, index):
        ...
        relative_audio_path = record["audio_path"]  # .arrow中的相对路径
        class_name = record["instrument_family_str"]                 

        # 拼接绝对音频路径：base_audio_dir + relative_audio_path
...
class NSynthIdataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            self.train_dataset = NSynthIdataset(dataset_path=self.train_audio_dir, **self.dataset_args)
            self.valid_dataset = NSynthIdataset(dataset_path=self.valid_audio_dir, **self.dataset_args)
        if stage == "val":
            self.valid_dataset = NSynthIdataset(dataset_path=self.valid_audio_dir, **self.dataset_args)
        if stage == "test":
            self.test_dataset = NSynthIdataset(dataset_path=self.test_audio_dir, **self.dataset_args)
