...
class LibriTTS_ctc_module(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            self.train_dataset = LibriTTS_ctc_dataset(self.train_audio_dir, base_audio_dir=self.base_audio_dir)
            self.valid_dataset = LibriTTS_ctc_dataset(self.valid_audio_dir, base_audio_dir=self.base_audio_dir)
        if stage == "val":
            self.valid_dataset = LibriTTS_ctc_dataset(self.valid_audio_dir, base_audio_dir=self.base_audio_dir)
        if stage == "test":
            self.test_dataset = LibriTTS_ctc_dataset(self.test_audio_dir, base_audio_dir=self.base_audio_dir)
