...
from codec_evaluation.probe.dataset.MTG_dataset.MTGInstrument_dataset import MTGInstrumentdataset

root_path = codec_evaluation.__path__[0]
logger = RankedLogger(__name__, rank_zero_only=True)

@hydra.main(config_path=f"{root_path}/probe/config/MTG_dataset/MTGInstrument_dataset", config_name="semanticodec.yaml", version_base=None) # 需要更改
def main(config: DictConfig) -> None:
    ...
    logger.info(f"ap: {model.test_step_outputs['ap']}")
    logger.info(f"aucroc: {model.test_step_outputs['aucroc']}")
    # # 保存结果
