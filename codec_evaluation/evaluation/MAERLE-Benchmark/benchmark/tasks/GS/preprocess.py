    ...
    for split_name in ["train", "test"]:
        ...
        for path in pathlib.Path(code, "md5").glob("*.md5"):
            ...
            if clip_duration is not None:
                ...
                if status != 0:
                    print(f"Error running ffprobe: {stderr}")
                assert status == 0
            ...
            if len(example) == 2:
                ...
                if src_audio_path is not None:
                    ...
                    if dest_audio_path.exists():
                        # print(f"File {dest_audio_path} already exists. Skipping.")
                        continue  # 跳过已存在的文件
                    ...
                    try:
                        ...
                        print(f"Error processing file {src_audio_path}: {e}")
                        continue  # 跳过错误文件
            else:
                raise ValueError("Bad iterator")

            if uid in all_metadata:
                raise ValueError("Duplicate UID in iterator")
